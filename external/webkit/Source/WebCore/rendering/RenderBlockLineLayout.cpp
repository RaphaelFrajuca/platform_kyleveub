...
#define TAMIL_LETTER_KA 2965
#define TAMIL_SIGN_VIRAMA 3021
#define TAMIL_LETTER_SSA 2999


#define TAMIL_LETTER_SA 3000
#define TAMIL_LETTER_RA 2992
#define TAMIL_VOWEL_SIGN_II 3008

namespace WebCore {

// We don't let our line box tree for a single line get any deeper than this.
const unsigned cMaxLineDepth = 200;
class MoveComplexCharacterToNextLine{
	private:
		const UChar* mStr;
		const int mLength;
		bool combinationOfThree(int _pos)
		{
			return (int)mStr[_pos]== TAMIL_LETTER_SSA && (int)mStr[ _pos-1]== TAMIL_SIGN_VIRAMA && (int)mStr[_pos-2]==TAMIL_LETTER_KA;
		}
		bool combinationOfFour(int _pos)
		{
			bool result=false;  
			if( _pos<mLength )        
				result=(int)mStr[_pos]== TAMIL_LETTER_RA  && (int)mStr[_pos-1] == TAMIL_SIGN_VIRAMA  && (int)mStr[_pos-2]== TAMIL_LETTER_SA && (int)mStr[_pos+1]== TAMIL_VOWEL_SIGN_II;
			return result;
		}
	public:
		MoveComplexCharacterToNextLine(const UChar* _Str,const int _len):mStr(_Str),mLength(_len){
		}
		int indexToMoveTheCompositeCharacterToNextLine(int _pos){
			int lineBreakIndex=0;
			if(((int)mStr[_pos]== TAMIL_LETTER_SSA) && ((mLength>=3)&&combinationOfThree(_pos)))
				lineBreakIndex=2;	
			else if(((int)mStr[_pos]== TAMIL_LETTER_RA) && (((mLength>=4))&&combinationOfFour(_pos)))
				lineBreakIndex=2; 
...
void RenderBlock::layoutInlineChildren(bool relayoutChildren, int& repaintLogicalTop, int& repaintLogicalBottom)
{
    ...
    if (firstChild()) {
	    ...
	    if(end.m_nextBreakablePosition>0) {
		    RenderObject* o = resolver.position().m_obj;
		    if(o && o->isText()){
			    RenderText *t = toRenderText(o);
			    MoveComplexCharacterToNextLine LB(t->characters(),t->textLength());
			    int result=LB.indexToMoveTheCompositeCharacterToNextLine(end.m_pos);
			    end.m_pos = end.m_pos-result;
			    end.m_nextBreakablePosition = end.m_nextBreakablePosition - result;
